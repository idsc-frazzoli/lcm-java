...
public class ObjectPanel extends JPanel {
    ...
    class PaintState {
        Color indentColors[] = new Color[] { new Color(255, 255, 255), new Color(230, 230, 255),
        ...
        public void drawStringsAndGraph(Class cls, String name, Object o, boolean isstatic, int sec) {
            ...
            if (cls.equals(Byte.TYPE)) {
                g.drawString(String.format("0x%02X   %03d   %+04d   %c", (o), ((Byte) o).intValue() & 0x00FF, (o),
                        ((Byte) o) & 0xff), x[2], y);
            } else {
        ...
        public void DrawSparkline(int x, int y, ITrace2D trace, boolean isHovering) {
            ...
            if (trace.getMaxY() == trace.getMinY()) {
                ...
                g2.drawLine(leftLineX, y - (int) ((double) height / (double) 2), x + (int) width,
    ...
    public void paint(Graphics g) {
        ...
        if (!visibleSparklinesInitialized && visibleSparklines.isEmpty() && (previousNumSections > 0)
                && (scrollViewport != null)) {
            ...
            for (Field f : fs) {
                try {
                    paintRecurse(g, ps, f.getName(), f.getType(), f.get(o),
                            isstatic || ((f.getModifiers() & Modifier.STATIC) != 0), sec);
                } catch (Exception ex) {
    ...
    void UpdateGraphDataWithoutPaintRecurse(SparklineData sparklineToUpdate, String name, Class cls, Object o) {
        ...
        if (cls.isPrimitive() || cls.equals(Byte.TYPE)) {
            ...
            for (int i = 0; i < sz; i++)
                UpdateGraphDataWithoutPaintRecurse(sparklineToUpdate, name + "[" + i + "]", cls.getComponentType(),
                        Array.get(o, i));
