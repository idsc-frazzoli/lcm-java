 ...
 * {@link LCMSubscriber#messageReceived messageReceived} method on a subscriber
 * as soon as a message is received. This class provides an alternate way to
 * receive messages by storing them in an internal queue, and then delivering
 * them to synchronously to the user.
 * <p>
...
public class MessageAggregator implements LCMSubscriber {
   ...
   * aggregator. Messages are discarded oldest-first to ensure that the total
   * size of unretrieved messages stays under this limit.
   *
   * @param val
  ...
  /** Retrieves the maximum amount of memory that will be used to store
   * messages. */
   ...
   * 
   * @param timeout_ms
   * Max # of milliseconds to wait for a message. If 0, then don't
   * wait. If less than 0, then wait indefinitely.
